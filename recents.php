	...
	function widget( $args, $instance ) {
		...
		if ( $comments ) {
			...
			foreach ( (array) $comments as $comment) {
                            ...
                            $output .=  '<li class="recentcomments">' . /*translators: comments widget: 1: comment author, 2: post link*/  sprintf(_x(' %1$s on %2$s <br>  %3$s <br> <br> %4$s', 'widgets'), get_comment_author_link(), '<a href="' . esc_url( get_comment_link($comment->comment_ID) ) . '">' . $posttitle . '</a>',$com,$image) . '</li>';
